...
class ImprovedCloudTrailLogsPerformanceTester:
    ...
    def parse_cloudtrail_queries_file(self, file_path):
        """Parse CloudTrail queries file"""
    ...
    def run_cloudtrail_performance_test(self, query_data):
        ...
        print(f"\nTesting CloudTrail Query #{query_id}...")
...
def main():
    ...
    for query_data in queries:
        try:
            result = tester.run_cloudtrail_performance_test(query_data)
            results.append(result)
        except KeyboardInterrupt:
            print("\nTest interrupted by user")
            break
        except Exception as e:
            print(f"Unexpected error testing CloudTrail query #{query_data['id']}: {e}")
            traceback.print_exc()
            continue
